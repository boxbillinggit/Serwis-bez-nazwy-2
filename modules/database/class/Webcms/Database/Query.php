...
class Webcms_Database_Query {
	...
	final public function __toString()
	{
		...
		{
			return Webcms::exception_text($e);
		}
	}
	...
	public function execute($db = 'default')
	{
		...
		if ( ! empty($this->_lifetime) AND $this->_type === Database::SELECT)
		{
			...
			if ($result = Webcms::cache($cache_key, NULL, $this->_lifetime))
			{
		...
		if (isset($cache_key))
		{
			...
			Webcms::cache($cache_key, $result->as_array(), $this->_lifetime);
		}
